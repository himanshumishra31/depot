...
class PriceValidator < ActiveModel::EachValidator
  def validate_each(record, attribute, value)
    record.errors[attribute] << "Price should be greater than discount price" if record.price < record.discount_price
  end
end
...
class Product < ApplicationRecord
  ...
  before_validation :default_initialization, unless: :title?
  # before_save :set_discout_price_to_price, unless:
  ...
  private
    ...
    def price_cannot_be_less_than_discount_price
      errors.add(:price, "should be greater than discount_price") if price < discount_price
    end
